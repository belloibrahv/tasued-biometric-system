...
# Script to add lecturers to the system
# This script adds the two lecturer accounts to both Supabase Auth and the database

echo "🔧 Fixing Lecturer Accounts..."
echo ""

# Check if environment variables are loaded
if [ -z "$NEXT_PUBLIC_SUPABASE_URL" ]; then
  echo "❌ Error: Environment variables not loaded"
  echo "Please ensure .env or .env.local is configured"
  exit 1
fi
...
# Create a Node.js script to add lecturers
cat > /tmp/add-lecturers-fix.js << 'SCRIPT'
